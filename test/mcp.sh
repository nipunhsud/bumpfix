...
OUT=$(printf '%s\n%s\n%s\n' \
  ...
  '{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"upgrade_dependency","arguments":{"package":"isarray","version":"1.0.0$(touch /tmp/bumpfix-pwned)","cwd":"/tmp"}}}' \
  'not json at all' \
  | node "$MCP")
...
[ ! -f /tmp/bumpfix-pwned ] || { echo "FAIL: injection executed"; rm -f /tmp/bumpfix-pwned; exit 1; }
